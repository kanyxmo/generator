...
use Mine\Devtool\Generator\ProducerGen;
...
{
    protected ?string $name = 'gen:producer';

    public function handle()
    {
        try {
            foreach ((array) $this->getOptionInput('name', []) as $name) {
                $result = (new ProducerGen($this->getAppInput(), $name))->generate();
                $this->info(
       ...
       {
           $this->setDescription('创建新的MQ[amqp、kafka、nats、nsq]生产者类');
