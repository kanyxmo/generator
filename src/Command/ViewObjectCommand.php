...
use Mine\Devtool\Generator\ViewObjectGen;
use Hyperf\Stringable\Str;

#[Command]
class ViewObjectCommand extends GeneratorCommand
{
    protected ?string $name = 'gen:vo';

    public function handle()
    {
        try {
            foreach ((array) $this->getOptionInput('name', []) as $name) {
                $result = (new ViewObjectGen($this->getAppInput(), $name))->generate();
                $this->info(
