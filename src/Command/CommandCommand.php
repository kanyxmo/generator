...
use Mine\Devtool\Generator\CommandGen;
    ...
    public function handle()
    {
        try {
            foreach ((array) $this->getOptionInput('name', []) as $name) {
                $result = (new CommandGen($this->getAppInput(), $name))->preview();
                var_dump(...$result);
                // $this->info(sprintf('%s %s 构建成功', Str::replaceLast('Command', '', Arr::last(explode('\\', static::class))), $result));
            }
