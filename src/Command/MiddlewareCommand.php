...
use Mine\Devtool\Generator\MiddlewareGen;
use Hyperf\Stringable\Str;

#[Command]
class MiddlewareCommand extends GeneratorCommand
{
    protected ?string $name = 'gen:middleware';

    public function handle()
    {
        try {
            foreach ((array) $this->getOptionInput('name', []) as $name) {
                $result = (new MiddlewareGen($this->getAppInput(), $name))->generate();
                $this->info(
